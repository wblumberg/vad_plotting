...
class VADProfile(Profile):
    '''
        This is the data class for VAD profiles, for example those from the
        WSR-88D radars.  It inherits from Profile and holds a couple of different
        data types.  It contains dummy temperature, humidity and pressure
        profiles.
    '''
    def __init__(self, **kwargs):
        '''
    ...
    def get_sfc(self):
        '''
            ...
            '''
        return np.where(~self.wspd.mask)[0].min()
    ...
    def get_top(self):
        '''
            ...
            Index of the surface
            '''
        return np.where(~self.wspd.mask)[0].max()
